# distutils: sources = hacl-packages/src/Hacl_Curve25519_64.c

from cython.cimports.libc.stdint import uint8_t
from cython.cimports.pyhacl.key_exchange import curve25519_64
...
def scalarmult(secret_key: bytes, public_key: bytes) -> bytes:
    ...
    point: uint8_t[32]
    curve25519_64.Hacl_Curve25519_64_scalarmult(point, skey, pkey)
    return point[:32]
...
def secret_to_public(secret_key: bytes) -> bytes:
    ...
    skey: uint8_t[32] = secret_key
    pkey: uint8_t[32]
    curve25519_64.Hacl_Curve25519_64_secret_to_public(pkey, skey)
    return pkey[:32]
...
def ecdh(secret_key: bytes, public_key: bytes) -> bytes:
    ...
    if not curve25519_64.Hacl_Curve25519_64_ecdh(shared_key, skey, pkey):
        e = 'invalid key'
        raise ValueError(e)
